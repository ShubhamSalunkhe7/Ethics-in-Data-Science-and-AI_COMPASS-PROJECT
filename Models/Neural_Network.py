...
# This step is CRITICAL for neural networks
# Unlike tree-based models (Random Forest, XGBoost) which
# split on thresholds and do not care about scale,
# neural networks use WEIGHTS — tiny numbers that multiply
# your features. If age=25 and priors=3 are on different
# scales, the weights become unbalanced and learning breaks.
#
...
# Now both are on the same small scale — weights work properly
#
# RULE: Always fit the scaler on TRAINING data only
# then use it to transform BOTH train and test
# Fitting on test data would be "cheating" —
# the model would have seen future information
...
# This creates THREE hidden layers:
#   Layer 1: 128 neurons  (learns basic patterns)
#   Layer 2: 64 neurons   (learns more complex patterns)
#   Layer 3: 32 neurons   (learns most complex patterns)
#
...
# Simple rule: if the signal is positive, pass it through
#              if the signal is negative, block it (set to 0)
# This prevents the network from becoming stuck
# and allows it to learn complex non-linear patterns
#
# solver='adam'
# Adam is the learning algorithm (optimiser)
# It adjusts all the weights in the network after
# each batch of training examples
# Adam is clever because it adapts its learning speed
# automatically — fast when uncertain, slow when confident
#
# alpha=0.001
